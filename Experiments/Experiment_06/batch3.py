...
app = modal.App("tcc-final-schrod-v22-b3", image=image)
...
def train_one(
    ...
    print(
        f"[{label}] start: n_params={n_params} device={device} "
        f"kind={kind} N_L={n_layers} W={width} iters={iterations}",
        flush=True,
    )

    # Periodic save to avoid losing work on timeout
    def save_partial(ep_now, u_pred, v_pred, abs_p, l2_val, l2_u, l2_v, wall_so_far, final_loss):
        record = {
            "label": label, "kind": kind, "partial": True, "ep_at_save": ep_now,
            ...
            "final_loss": final_loss, "wall_clock": wall_so_far,
    ...
    SAVE_EVERY = 2000  # checkpoint cadence
    ...
    for ep in range(iterations + 1):
        ...
        if ep % EPOCHS_LOG == 0:
            ...
            print(
                ...
                f"L2|psi|={l2_now:.3e} t={time.perf_counter()-t0:.0f}s",
                flush=True,
            )

        # Periodic checkpoint (save current full prediction)
        if ep > 0 and ep % SAVE_EVERY == 0:
            ...
            save_partial(ep, u_p, v_p, abs_p_, l2v, l2u_, l2v_,
                         time.perf_counter() - t0, float(loss.item()))
    ...
    record = {
        "label": label, "kind": kind, "partial": False,
        "n_layers": n_layers, "width": width, "mode": "nsup",
        "seed": seed, "lr": lr, "iterations": iterations,
        "lambda_ic": LAMBDA_IC, "lambda_bc": LAMBDA_BC,
        "n_params": n_params,
    ...
    print(
        f"[{label}] DONE L2|psi|={l2_val:.3e} L2u={l2_u:.3e} L2v={l2_v:.3e} "
...
def main() -> None:
    ...
    # Mix configs sized to fit comfortably in 1700s timeout — small archs only.
    # Plus matching PINN 20k baseline (we have 8x100 20k already; add 4x64 20k
    # for size-matched comparison).
    configs = [
        # (label, kind, n_layers, width, seed, lr, iters)
        ("mix_2x2_sof_nsup_15k",  "mix",  2, 2,  21, LR_MIX,  15000),
        ("mix_3x3_sof_nsup_12k",  "mix",  3, 3,  21, LR_MIX,  12000),
        ("mix_3x6_sof_nsup_10k",  "mix",  3, 6,  21, LR_MIX,  10000),
        ("mix_4x4_sof_nsup_10k",  "mix",  4, 4,  21, LR_MIX,  10000),
        # extra PINN to compare at matched iters with whichever Mix wins
        ("pinn_8x100_nsup_15k",   "pinn", 8, 100, 21, LR_PINN, 15000),
    ]

    print(f"[main] Batch 3: {len(configs)} configs", flush=True)
    t0 = time.perf_counter()
    results = list(train_one.starmap(configs))
    wall = time.perf_counter() - t0
    print(f"[main] {len(results)} finished in {wall:.1f}s", flush=True)

    print(save_results.remote(results, "b3"), flush=True)
