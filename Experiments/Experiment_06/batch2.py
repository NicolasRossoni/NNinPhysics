...
app = modal.App("tcc-final-schrod-v22-b2", image=image)
...
def train_one(
    ...
    def make_pools(t_top):
        x_int_pool = (tc.rand(N_INT, 1, generator=g_cpu) * (X_MAX - X_MIN) + X_MIN).to(device)
        t_int_pool = (tc.rand(N_INT, 1, generator=g_cpu) * (t_top - T_MIN) + T_MIN).to(device)
        t_bc = (tc.rand(N_BC, 1, generator=g_cpu) * (t_top - T_MIN) + T_MIN).to(device)
        x_bc_left = tc.full((N_BC, 1), X_MIN, device=device)
        x_bc_right = tc.full((N_BC, 1), X_MAX, device=device)
        return x_int_pool, t_int_pool, t_bc, x_bc_left, x_bc_right

    # Curriculum: half iters at T_MAX/2, then full
    cur_split = iterations // 2 if curriculum else 0
    if curriculum:
        x_int_pool, t_int_pool, t_bc, x_bc_left, x_bc_right = make_pools(T_MAX / 2.0)
    else:
        x_int_pool, t_int_pool, t_bc, x_bc_left, x_bc_right = make_pools(T_MAX)

    print(
        f"[{label}] start: n_params={n_params} device={device} "
        f"kind={kind} N_L={n_layers} W={width} curriculum={curriculum}",
        flush=True,
    )

    for ep in range(iterations + 1):
        if curriculum and ep == cur_split:
            x_int_pool, t_int_pool, t_bc, x_bc_left, x_bc_right = make_pools(T_MAX)
            print(f"[{label}] curriculum: switch to full t-domain at ep={ep}", flush=True)
...
def main() -> None:
    ...
    configs = [
        # (label, kind, n_layers, width, seed, lr, iters, curriculum)
        ("pinn_8x100_nsup_40k",      "pinn", 8, 100, 21, LR_PINN, 40000, False),
        ("pinn_6x100_nsup_30k",      "pinn", 6, 100, 21, LR_PINN, 30000, False),
        ("pinn_8x100_nsup_40k_curr", "pinn", 8, 100, 21, LR_PINN, 40000, True),
        ("mix_3x3_sof_nsup_20k",     "mix",  3, 3,   21, LR_MIX,  20000, False),
        ("mix_3x6_sof_nsup_15k",     "mix",  3, 6,   21, LR_MIX,  15000, False),
        ("mix_4x4_sof_nsup_15k",     "mix",  4, 4,   21, LR_MIX,  15000, False),
    ]

    print(f"[main] Batch 2: {len(configs)} configs", flush=True)
    t0 = time.perf_counter()
    results = list(train_one.starmap(configs))
    wall = time.perf_counter() - t0
    print(f"[main] {len(results)} finished in {wall:.1f}s", flush=True)

    print(save_results.remote(results, "b2"), flush=True)
