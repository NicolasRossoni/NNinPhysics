...
app = modal.App("tcc-kov-v13", image=image)
volume = modal.Volume.from_name("tcc", create_if_missing=True)

VOLUME_PATH = "/data"
EXPERIMENT = "final/kov_v13"

RE = 40.0
LAM = RE / 2.0 - math.sqrt((RE / 2.0) ** 2 + 4.0 * math.pi ** 2)
X_MIN, X_MAX = -0.5, 1.0
Y_MIN, Y_MAX = -0.5, 1.5
X_INFER_MAX = 2.5  # inferencia estendida em x
...
@app.function(gpu="T4", timeout=120 * 60, volumes={VOLUME_PATH: volume}, max_containers=30)
def train_one(
    label: str,
    kind: str,
    n_layers: int,
    ...
    if bc_mode == "leak":
        # MODO ANTIGO (v12): u_bc = kovasznay interior -> vaza analitica.
        def u_bc_fn(xy):
            return kovasznay(xy[:, 0:1], xy[:, 1:2])
    elif bc_mode == "coons":
        # Interpolacao transfinita de Coons.
        # u_bc(x,y) = (1-xi)*u_W(y) + xi*u_E(y) + (1-eta)*u_S(x) + eta*u_N(x)
        #            - corner_corr
        def u_bc_fn(xy):
            x = xy[:, 0:1]; y = xy[:, 1:2]
            xi = (x - X_MIN) / (X_MAX - X_MIN)
            eta = (y - Y_MIN) / (Y_MAX - Y_MIN)
            xm_v = tc.full_like(x, X_MIN)
            xM_v = tc.full_like(x, X_MAX)
            ym_v = tc.full_like(y, Y_MIN)
            yM_v = tc.full_like(y, Y_MAX)
            # Bordas (kovasznay avaliada nos lados)
            uw, vw, pw = kovasznay(xm_v, y)
            ue, ve, pe = kovasznay(xM_v, y)
            us, vs, ps = kovasznay(x, ym_v)
            un, vn, pn = kovasznay(x, yN := yM_v)
            # Cantos
        ...
        raise ValueError(f"bc_mode invalido: {bc_mode}")
    ...
    net.eval()
    # ---- Pruning iterativo (apenas se solicitado) ----
    prune_results = []
    if do_pruning and prune_ratios and kind == "mix":
        with tc.no_grad():
            for r in prune_ratios:
                ...
                # Restaura alphas para proxima iteracao (cumulativo)
                for n, p in net.named_parameters():
                    if "alpha" in n and n in state_before:
                        p.data.copy_(state_before[n])

    # Avaliacao final no grid 80x100 sem prune
    nx_e, ny_e = 80, 100
    xe = tc.linspace(X_MIN, X_MAX, nx_e, device=device)
    ye = tc.linspace(Y_MIN, Y_MAX, ny_e, device=device)
    XE, YE = tc.meshgrid(xe, ye, indexing="xy")
    ...
    # Inferencia em X estendido [-0.5, 2.5]
    nx_xe, ny_xe = 200, 120
    ...
    with tc.no_grad():
        u_ex, v_ex, p_ex = forward_hard(xy_ex)
        u_an, v_an, p_an = kovasznay(XEX.reshape(-1, 1), YEX.reshape(-1, 1))
    extrap_x_grid = {
    ...
    print(f"[{label}] bc={bc_mode} L2={l2_val:.3e} wall={wall:.1f}s n={n_params}", flush=True)

    out_record = {
        "label": label, "kind": kind, "n_layers": n_layers, "width": width,
        "mode": mode, "seed": seed, "dropout": dropout, "subsample": subsample,
        "lr": lr, "iterations": iterations, "bc_mode": bc_mode,
    ...
    try:
        ...
        if save_state_dict:
            sd_path = out_dir / f"{label}.statedict.pt"
            tc.save(net.state_dict(), sd_path)
        volume.commit()
    except Exception as e:
        print(f"[{label}] WARN commit failed: {e}", flush=True)
...
def main() -> None:
    ...
    # ===== LOTE A: Tabela 1 (sup/nsup x PINN/Mix x 3 seeds, BC LEAK) =====
    for mode in ["sup", "nsup"]:
        for seed in [21, 22, 23]:
            configs.append((
                f"pinn_4x32_{mode}_s{seed}", "pinn", 4, 32, mode, seed,
                0.0, 1.0, LR_PINN, ITER, "leak", False, False, None,
            ))
    for mode in ["sup", "nsup"]:
        for seed in [21, 22, 23]:
            configs.append((
                f"mix_3x1_{mode}_s{seed}", "mix", 3, 1, mode, seed,
                0.0, 1.0, LR_MIX, ITER, "leak", False, False, None,
            ))

    # ===== LOTE B: Regularizacao Mix 3x1 com BC hard CORRETA (Coons) =====
    configs.append(("reg_mix_baseline", "mix", 3, 1, "nsup", 21, 0.00, 1.0, LR_MIX, ITER, "coons", False, False, None))
    configs.append(("reg_mix_drop10",   "mix", 3, 1, "nsup", 21, 0.10, 1.0, LR_MIX, ITER, "coons", False, False, None))
    configs.append(("reg_mix_drop15",   "mix", 3, 1, "nsup", 21, 0.15, 1.0, LR_MIX, ITER, "coons", False, False, None))
    configs.append(("reg_mix_sub70",    "mix", 3, 1, "nsup", 21, 0.00, 0.7, LR_MIX, ITER, "coons", False, False, None))
    configs.append(("reg_mix_sub50",    "mix", 3, 1, "nsup", 21, 0.00, 0.5, LR_MIX, ITER, "coons", False, False, None))
    configs.append(("reg_mix_combo",    "mix", 3, 1, "nsup", 21, 0.10, 0.5, LR_MIX, ITER, "coons", False, False, None))

    # ===== LOTE C: Pruning novo Mix 3x1 sof=False com BC Coons + dump state_dict =====
    # 1 treino base que aplica pruning iterativo internamente em multiplos ratios
    prune_ratios = [0.0, 0.3, 0.5, 0.7, 0.9]
    configs.append((
        "prune_mix_3x1_coons", "mix", 3, 1, "nsup", 21,
        0.0, 1.0, LR_MIX, ITER, "coons", True, True, prune_ratios,
    ))

    print(f"[main] launching {len(configs)} jobs Modal T4...", flush=True)
    t0 = time.perf_counter()
    results = list(train_one.starmap(configs))
    wall = time.perf_counter() - t0
    print(f"[main] {len(results)} done in {wall:.1f}s", flush=True)
    Path("/tmp/kov_v13_results.json").write_text(json.dumps(results, indent=2))
    print("[main] saved /tmp/kov_v13_results.json", flush=True)
