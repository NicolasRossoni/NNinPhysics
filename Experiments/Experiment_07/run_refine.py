...
app = modal.App("tcc-baldan-v23-refine", image=image)
...
def train_one(
    ...
    best = {"l2x": float("inf"), "l2z": float("inf"), "l2_tot": float("inf"), "state": None}
    ...
    for k in range(lbfgs_outer):
        ...
        if k % max(1, lbfgs_outer // 30) == 0 or k == lbfgs_outer - 1:
            ...
            print(f"[{label}] lbfgs k={k} L={state['L'].item():.3e} "
                  ...
                  f"L2x={l2x:.3e} L2z={l2z:.3e} (best L2_tot={best['l2_tot']:.3e})", flush=True)
...
def main():
    # 3 jobs refinados: PINN 6x64 com 300 LBFGS outer, PINN 8x96 com 200, Mix 3x6 sof
    cfg = [
        ("raissi_lift_6x64_long", "pinn", 6, 64, 21, 1e-3, 5000, 300, 50, False),
        ("raissi_lift_8x96",      "pinn", 8, 96, 21, 1e-3, 5000, 200, 50, False),
        ("mix_3x6_sof_lift",      "mix",  3,  6, 21, 1e-3, 5000, 200, 50, True),
    ]
    print(f"[main] launching {len(cfg)} jobs v23 refine...", flush=True)
    t0 = time.perf_counter()
    ...
    Path("/tmp/baldan_v23_refine_results.json").write_text(json.dumps(results, indent=2))
